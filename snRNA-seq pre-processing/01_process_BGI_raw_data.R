...
sample_metadata <- read.csv("./02_sample_info_BGI.csv")
...
create_seurat_from_sample <- function(sample_id, run_id, sample_metadata) {
  data_directory <- file.path("./00_BGI_matrix", sample_id, run_id)
  expression_data <- Read10X(data_directory, gene.column = 1)
...
BGI_budgerigar_brain_obj <- processed_samples[[1]]

# Merge all individual Seurat objects into the final combined object
for (i in 2:length(processed_samples)) {
  BGI_budgerigar_brain_obj <- merge(BGI_budgerigar_brain_obj, y = processed_samples[[i]])
}

saveRDS(BGI_budgerigar_brain_obj, file = '03_BGI_budgerigar_brain_obj.rds')
