...
rctd_results = pd.read_csv('/data/work/RCTD/9mm/9mm_norm_weights.csv', index_col=0)
rctd_results.index = rctd_results.index.str.replace('^X', '', regex=True)
...
#fliter
spatial_coords = adata.obsm['spatial']
mask = spatial_coords[:, 0] >= -8300
adata = adata[mask, :]  

#save
adata.write("/data/output/9mm_Tel_RCTD.h5ad")